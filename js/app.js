...
function mergeLiveStats(text) {
  ...
  rows.forEach(function(r) {
    ...
    DATA.stats.push({
      ...
      player_id:nm,
      G:n(r['G']),AB:n(r['AB']),R:n(r['R']),H:n(r['H']),RBI:n(r['RBI']),
      dbl:n(r['2B']),trp:n(r['3B']),HR:n(r['HR']),BB:n(r['BB']),
      ...
      pit_RA:n(r['R_1']),pit_W:n(r['W']),pit_L:n(r['L']),RIP:n(r['RIP']),pit_S:null,
    });
  });
  console.log('Live stats merged:', rows.length);
}
...
function mergeLiveBox(text) {
  ...
  console.log('Live box merged:', DATA.logs.filter(function(l){return l.live;}).length);
}
...
function fmtStat(v,d) { d=d||0; return (v===null||v===undefined)?'—':Number(v).toFixed(d); }
function fmtBA(v) { return (v===null||v===undefined)?'—':Number(v).toFixed(3).replace(/^0\./,'.'); }
function fmtRV(v,d) { d=d||1; return (v===null||v===undefined)?'—':Number(v).toFixed(d); }
...
function makeAvatarImg(id, size) {
  size = size || '100%';
  var lo = id.toLowerCase();
  var style = 'width:'+size+';height:'+size+';object-fit:cover;border-radius:50%';
  // onerror chain: .jpg -> .jpeg -> lowercase.jpg -> lowercase.jpeg -> fallback
  var err = 'if(!this.dataset.t)this.dataset.t=0;' +
    'this.dataset.t=+this.dataset.t+1;' +
  ...
  return '<img src="img/players/' + id + '.jpg" style="' + style + '" onerror=\'' + err + '\' alt="">';
}

function makeLeaderPhoto(id) {
  var lo = id.toLowerCase();
  var style = 'width:30px;height:30px;object-fit:cover;border-radius:50%;border:1px solid var(--border-bright);flex-shrink:0';
  var err = 'if(!this.dataset.t)this.dataset.t=0;' +
    'this.dataset.t=+this.dataset.t+1;' +
    'var t=+this.dataset.t;' +
    'if(t===1)this.src="img/players/' + id + '.jpeg";' +
...
function buildBoxTableWithPos(rows, showDateOpp) {
  var nameCol = showDateOpp
    ...
    : '<th style="text-align:left">Player</th>';
  var bodyRows = rows.map(function(l) {
    var posStr = POS_COLS.filter(function(k){return l[k]&&l[k]>0;}).map(function(k){return POS_LABELS[k];}).join('/') || '—';
    var fc = showDateOpp
      ? '<td style="text-align:left">'+l.date+'</td><td style="text-align:left">'+(l.opponent||'—')+'</td>'
      : '<td style="text-align:left"><a onclick="navigate(\'profile\',\''+l.player_id+'\')">'+displayName(l.player_id)+'</a></td>';
...
function renderSeasonLeaders() {
  ...
  var lbl = seasonLabel(DATA.maxSeason);
  ...
  function leaderCard(stat, valFn, fmtFn, qualifier) {
    ...
    var photoHTML;
    if (leaders.length>=3) {
      photoHTML = '<img src="img/logo.png" style="width:30px;height:30px;object-fit:contain;border-radius:50%;background:white;padding:3px;flex-shrink:0" alt="">';
    } else {
...
function renderHomeGames() {
  ...
  if (latest) {
    ...
    recentHTML = '<div class="section-title" style="margin-top:1.25rem">Last Game \u2014 '+latest.date+' vs '+latest.opponent+'</div>'+
...
function renderGameList() {
  ...
  games.forEach(function(g){
    ...
    html+='<div class="card mb1" style="cursor:pointer" onclick="toggleDetail(this)">'+
